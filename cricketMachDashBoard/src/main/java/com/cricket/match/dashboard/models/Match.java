package com.cricket.match.dashboard.models;


import com.cricket.match.dashboard.UI.Dashboard;
import com.cricket.match.dashboard.enums.MatchStatus;
...
public class Match {
	...
	private void startBatting() {
		System.out.println(
				"Batting team is : " + this.currentBattingTeam.getTeamName() + " playing for overs : " + this.numOfOversInMatch);
		...
		for (int i = 1; i <= this.numOfOversInMatch; i++) {
			...
			for (int j = 0; j < 6; j++) {
				...
				if (!updateScore(
						score) || (previousBattingTeam != null && this.currentBattingTeam.getTotalTeamScore() >= this.previousBattingTeam.getTotalTeamScore())) {
